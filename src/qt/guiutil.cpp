...
{
    // Convert Bigup:// to Bigup:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Bigup://"))
    {
        uri.replace(0, 12, "Bigup:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bigup.lnk";
}
...
{
    return GetAutostartDir() / "Bigup.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bigup-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Bigup-qt [" + tr("command-line options") + "]                     " + "\n";
